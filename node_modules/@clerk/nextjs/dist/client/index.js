...
function ClerkProvider({ children, ...rest }) {
    ...
    const { 
    // @ts-expect-error
    authServerSideProps, frontendApi, publishableKey, 
    // @ts-expect-error
    proxyUrl, 
    // @ts-expect-error
    domain, 
    ...
    return (react_1.default.createElement(clerk_react_1.ClerkProvider, { frontendApi: frontendApi || process.env.NEXT_PUBLIC_CLERK_FRONTEND_API || '', publishableKey: publishableKey || process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY || '', clerkJSUrl: clerkJSUrl || process.env.NEXT_PUBLIC_CLERK_JS, 
        // @ts-expect-error
        proxyUrl: proxyUrl || process.env.NEXT_PUBLIC_CLERK_PROXY_URL || '', domain: domain || process.env.NEXT_PUBLIC_CLERK_DOMAIN || '', isSatellite: isSatellite || process.env.NEXT_PUBLIC_CLERK_IS_SATELLITE === 'true', navigate: to => push(to), 
